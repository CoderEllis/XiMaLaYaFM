...
class LBFMFindRecommendCell: UITableViewCell {
    private let LBFMFindRecommendPicCellID = "LBFMFindRecommendPicCell"
    //头像
    ...
    private lazy var collectionView : UICollectionView = {
        ...
        collectionView.register(LBFMFindRecommendPicCell.self, forCellWithReuseIdentifier: LBFMFindRecommendPicCellID)
    ...
    var streamModel : LBFMFindRStreamList? {
        didSet {
            ...
            picView.kf.setImage(with: URL(string: model.avatar!))
            
            nameLabel.text = model.nickname
            zanLabel.text = "\(model.likesCount)"
            commnetLabel.text = "\(model.commentsCount)"
            ...
            dateLabel.text = updateTimeToCurrennTime(timeStamp: Double(CGFloat(model.issuedTs)))
            findRecommendSteam = model
            
            let picNum = findRecommendSteam?.picUrls?.count ?? 0
            var num:CGFloat = 0
...
extension LBFMFindRecommendCell: UICollectionViewDelegate, UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return findRecommendSteam?.picUrls?.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: LBFMFindRecommendPicCell = collectionView.dequeueReusableCell(withReuseIdentifier: LBFMFindRecommendPicCellID, for: indexPath) as! LBFMFindRecommendPicCell
        cell.picModel = findRecommendSteam?.picUrls?[indexPath.row]
        return cell
    }
