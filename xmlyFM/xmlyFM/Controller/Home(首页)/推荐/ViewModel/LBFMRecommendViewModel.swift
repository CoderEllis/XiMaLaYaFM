...
extension LBFMRecommendViewModel {
    ...
    // 每个分区显示item数量
    func numberOfItemsIn(_ section: NSInteger) -> NSInteger {
        return 1
    }
    //每个分区的内边距
    ...
    func sizeForItemAt(_ indexPath: IndexPath) -> CGSize {
        ...
        let itemNums = (homeRecommendList?[indexPath.section].list?.count)!/3
        let count = homeRecommendList?[indexPath.section].list?.count
