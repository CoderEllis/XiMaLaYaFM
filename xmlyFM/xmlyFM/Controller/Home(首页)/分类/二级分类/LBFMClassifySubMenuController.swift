...
class LBFMClassifySubMenuController: UIViewController {
    ...
    func setupHeaderView() {
        ...
        if !self.isVipPush {
            ...
            let categoryId = Keywords?.last?.categoryId
            viewControllers.insert(LBFMClassifySubRecommendController(categoryId: categoryId!), at: 0)
        }
        for vc in viewControllers {
            addChild(vc)
        }
        let pageView = DNSPageView(frame: CGRect(x: 0, y: LBFMNavBarHeight, width: ScreenWidth, height: ScreenHeight - LBFMNavBarHeight), style: style, titles: nameArray as! [String], childViewControllers: viewControllers)
