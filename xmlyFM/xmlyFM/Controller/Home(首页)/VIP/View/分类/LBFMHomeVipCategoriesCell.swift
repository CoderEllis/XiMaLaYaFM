...
extension LBFMHomeVipCategoriesCell: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.categoryBtnModel = categoryBtnList?[indexPath.row]
        return cell
    }
    
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let string = categoryBtnList?[indexPath.row].properties?.uri else {
            let id = "0"
        ...
        let url = categoryBtnList?[indexPath.row].url ?? ""
        delegate?.homeVipCategoriesCellItemClick(id: id, url: url, title: (categoryBtnList?[indexPath.row].title)!)
        
    }
    //"http://m.ximalaya.com/business-book-mobile-web-v2/?app=iting&version=6.5.63&impl=com.gemd.iting&ts=1553497423393"
