...
class LBFMListenSubscibeCell: UITableViewCell {
    ...
    var albumResults : AlbumResultsModel? {
        didSet {
            ...
            picView.kf.setImage(with: URL(string: model.albumCover!))
            titleLabel.text = model.albumTitle
            subLabel.text = model.trackTitle
            timeLabel.text = updateTimeToCurrennTime(timeStamp: Double(model.lastUpdateAt))
        }
    }
