...
private let LeftTableViewCellID = "LBFMOneKeyListenLTCell"
private let RightTableViewCellID = "LBFMListenChannelCell"
...
class LBFMListenMoreChannelController: UIViewController {
    ...
    private lazy var leftTableView : UITableView = {
        let tableView = UITableView(frame: CGRect(x: 0, y: 0, width: ScreenWidth * 0.2, height: ScreenHeight))
        tableView.register(LBFMOneKeyListenLTCell.self, forCellReuseIdentifier: LeftTableViewCellID)
    ...
    private lazy var rightTableView : UITableView = {
        let tableView = UITableView(frame: CGRect(x: ScreenWidth * 0.2, y: 0, width: ScreenWidth * 0.8, height: ScreenHeight))
        tableView.register(LBFMListenChannelCell.self, forCellReuseIdentifier: RightTableViewCellID)
        tableView.delegate = self
        tableView.dataSource = self
        tableView.separatorStyle = .none
        return tableView
    ...
    func setupLoadData() {
        ...
        if let mappedObject = JSONDeserializer<ChannelClassInfoModel>.deserializeModelArrayFrom(json: json["classInfos"].description) {
            channelClassInfo = mappedObject as? [ChannelClassInfoModel]
        }
    }
...
extension LBFMListenMoreChannelController: UITableViewDelegate, UITableViewDataSource {
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        return channelClassInfo?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return channelClassInfo?[section].channelInfos?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == leftTableView {
            let cell: LBFMOneKeyListenLTCell = tableView.dequeueReusableCell(withIdentifier: LeftTableViewCellID, for: indexPath) as! LBFMOneKeyListenLTCell
            ...
            let cell: LBFMListenChannelCell = tableView.dequeueReusableCell(withIdentifier: RightTableViewCellID, for: indexPath) as! LBFMListenChannelCell
            cell.selectionStyle = .none
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let titleName = (self.channelClassInfo?[section].className)!
        let count = (self.channelClassInfo?[section].channelInfos?.count)!
        ...
        let lineView = UIView()
        lineView.frame = CGRect(x: 80, y: 20, width: ScreenWidth * 0.8 - 100, height: 0.5)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == leftTableView {
            ...
            self.rightTableView.selectRow(at: rightMoveToIndexPath, animated: true, scrollPosition: .top)
        }
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == rightTableView {
            ...
            let topHeaderViewIndexPath = rightTableView.indexPathsForVisibleRows?.first
            ...
            if newRow != oldRow {
                let newCell: LBFMOneKeyListenLTCell = leftTableView.cellForRow(at: leftMoveToIndexPath) as! LBFMOneKeyListenLTCell
                newCell.titleLabel.textColor = LBFMButtonColor
                newCell.lineView.isHidden = false
            ...
            lastPath = leftMoveToIndexPath
            leftTableView.selectRow(at: leftMoveToIndexPath, animated: true, scrollPosition: .middle)
